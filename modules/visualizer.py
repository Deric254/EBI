...
def format_number(n):
    ...
    n = float(n)
    if abs(n) >= 1_000_000:
        formatted = f"{int(n/1_000_000)}M" if n % 1_000_000 == 0 else f"{n/1_000_000:.0f}M"
    elif abs(n) >= 1_000:
        formatted = f"{int(n/1_000)}K" if n % 1_000 == 0 else f"{n/1_000:.0f}K"
    else:
        formatted = f"{int(n)}" if n == int(n) else f"{n:.0f}"
    return formatted

def clean_bin_label(x):
    """Clean bin labels to show the full range in a readable format"""
    try:
        ...
        return format_number(float(x_str))
    except Exception:
        return str(x)
...
def show(df, title="📊 Visualize Data", key=None):
    ...
    if x_col == y_col and chart_type != "Pie":
        ...
        try:
            ...
            if enable_binning and pd.api.types.is_numeric_dtype(df[x_col]) and 'counts' in locals():
            ...
            if plot_df.empty:
                ...
                if chart_type == "Bar":
                    try:
                        ...
                        x_labels = [truncate_text(clean_bin_label(idx), 1) for idx in grouped.index]
                    ...
                    try:
                        ...
                        x_labels = [truncate_text(clean_bin_label(idx), 1) for idx in plot_df[x_col].unique()] if enable_binning and pd.api.types.is_numeric_dtype(df[x_col]) else plot_df[x_col].unique()
                    ...
                    try:
                        ...
                        if not cat_cols:
                            ...
                            x_labels = [truncate_text(clean_bin_label(c), 1) for c in heatmap_data.columns]
                    ...
                    try:
                        ...
                        x_labels = [truncate_text(clean_bin_label(idx), 1) for idx in pareto_data.index]
                                ...
                                for i, (x, y) in enumerate(chart_pairs[:max_charts]):
                                    ...
                                    try:
                                        ...
                                        if not plot_df.empty:
                                            ...
                                            x_labels = [truncate_text(clean_bin_label(idx), 1) for idx in grouped.index]
                                            bars = ax.bar(x_labels, grouped.values, color=colors[:len(grouped.index)])
                                            ax.set_ylabel(truncate_text(y, 2), fontsize=9)
                                            ax.set_xlabel(truncate_text(x, 2), fontsize=9)
                                            plt.xticks(rotation=45, ha='right', fontsize=8)
                                            plt.yticks(fontsize=8)
                                            ax.set_title(chart_title, fontweight='bold', fontsize=10, pad=15)
                                            ax.spines['top'].set_visible(False)
